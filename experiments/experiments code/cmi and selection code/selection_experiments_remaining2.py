...
def estimate_cmi(x, y, z, method, bootstrap, bootstrap_avg):
    def estimate_cmi_diff_classif():
        model1 = construct_model2(1, 1 + z.shape[1], output='sigmoid')
        model2 = construct_model2(1, z.shape[1], output='sigmoid')
    ...
    if method == 'diff_classif':
        return estimate_cmi_diff_classif()
    elif method == 'classif':
...
def selection_tasks(gamma, n, h, seed):
    ...
    selected = select(X, Y, 'opt', 'diff_classif', bootstrap=None, bootstrap_avg=None)
    results[gamma][h]['diff_classif'] = selected

    selected = select(X, Y, 'opt', 'classif', bootstrap=True, bootstrap_avg=False)
    results[gamma][h]['classif_b'] = selected

    selected = select(X, Y, 'opt', 'classif', bootstrap=True, bootstrap_avg=True)
    results[gamma][h]['classif_b_avg'] = selected

    dt = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')

    with open(f'results_selection_remaining2/sel_{gamma}_{n}_{h}_{seed}_{dt}', 'wb') as fd:
